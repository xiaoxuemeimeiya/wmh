...
//newHandle是操作对象通过移动自身上级跟着移动
	...
	if(template.ElementList.indexOf(elm)>=0||elm.className.indexOf("dragresize")>=0)
	{
	//alert(elm.className);
	while(elm)
		{
...
Canvas.prototype.select=function(newElement)
{ 
with(this)
	{
	if(!document.getElementById||!enabled)
...
Canvas.prototype.deselect=function(delHandles)
{
with(this)
	{
	if(!document.getElementById||!enabled)
...
Canvas.prototype.resizeHandleSet=function(elm,show)
{
	//获取控件的布局位置和大小
var top=0;
var left=0;
...
with(this)
	...
	if(!bod._handle_tr)
		...
		for(var h=0;h<handles.length;h++)
			...
			hCanvas.className="dragresize"+' '+myName+'-'+handles[h];
