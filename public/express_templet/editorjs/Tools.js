...
Button.prototype.OnClick=function(e)
{
	...
	if(this.isfile)
	{
		...
		this.file.onchange=function(){
			var upfile_name = $(this).attr('name');
			var obj2 = this;
			$.ajaxFileUpload({
				url:upload_url+'?user_type='+user_type+'&file_name='+upfile_name,
				type:'post',
				secureuri:false,
				fileElementId:upfile_name,
				dataType: 'json',
				success: function (data) {
					console.log(obj2.files[0].name);
					if(data.status == 'success') {
						var addr=window.URL.createObjectURL(obj2.files[0]);
						obj.eventClick(data.url);
					}
				}
			});
		}
