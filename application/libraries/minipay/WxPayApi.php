...
class WxPayApi
{
    ...
    /**
     *
     * 企业付款到零钱，transfers中partner_trade_no、body、total_fee、trade_type必填
     * appid、mchid、spbill_create_ip、nonce_str不需要填入
     * @param WxPayConfigInterface $config  配置对象
     * @param WxPayUnifiedOrder $inputObj
     * @param int $timeOut
     ...
     * @return 成功时返回，其他抛异常
     */
    public static function transfers($config, $inputObj, $timeOut = 30)
    {
        ...
        */
        ...
        $inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);//终端ip
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
        //$inputObj->SetSignType('MD5');//随机字符串

        $inputObj->SetSign($config);//签名
        //return $inputObj;
        $xml = $inputObj->ToXml();
        $startTimeStamp = self::getMillisecond();//请求开始时间
        $response = self::postXmlCurl($config, $xml, $url, true, $timeOut);//是否需要证书
        //$response = self::curl_post_ssl($url,$xml,30,[]);
        //var_dump($xml);var_dump($inputObj->GetSign()); var_dump($response);exit;
        $result = WxPayDataBaseSignMd5::Init($config, $response);
        self::reportCostTime($config, $url, $startTimeStamp, $result);//上报请求花费时间
        return $result;
    }
